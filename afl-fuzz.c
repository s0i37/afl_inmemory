...
int afl_data_fd = -1;   /* send mutated data to the instrumentation */
int afl_sync_fd = -1;   /* recieve result of execution from the instrumentation */
fd_set afl_sync_set;    /* recieve result of execution from the instrumentation - timeout implementation */
...
static u8 run_target(char** argv, u32 timeout, void* mem, u32 len) {
  ...
  if(afl_data_fd == -1)
    afl_data_fd = open("afl_data", O_WRONLY);   /* WAIT */
  write(afl_data_fd, mem, len); /* SIGPIPE */

  if(afl_sync_fd == -1)
  {
    afl_sync_fd = open("afl_sync", O_RDONLY);   /* WAIT */
    FD_ZERO(&afl_sync_set);
    FD_SET(afl_sync_fd, &afl_sync_set);
  }
  wait.tv_sec = (timeout / 1000);
  wait.tv_usec = (timeout % 1000) * 1000;
  rv = select(FD_SETSIZE, &afl_sync_set, NULL, NULL, &wait);   /* WAIT */
  if(rv == -1)
    return FAULT_ERROR;
  else if(rv == 0)
    return FAULT_TMOUT;
  else
    read(afl_sync_fd, result, 1);
